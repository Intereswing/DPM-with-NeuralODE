...
def get_logger(logpath, filepath, package_files=None,
...
def subsample_timepoints(data, time_steps, mask, n_tp_to_sample=None):
    ...
    if n_tp_to_sample > 1:
        ...
        for i in range(data.size(0)):
            # take mask for current training sample and sum over all features -- figure out which time points don't
            # have any measurements at all in this batch
...
def split_data_extrap(data_dict, dataset=""):
    ...
    split_dict = {"observed_data": data_dict["data"][:, :n_observed_tp, :].clone(),
                  ...
                  "tp_to_predict": data_dict["time_steps"][n_observed_tp:].clone(), "observed_mask": None,
                  "mask_predicted_data": None, "labels": None}
...
def split_data_interp(data_dict):
    ...
    split_dict = {"observed_data": data_dict["data"].clone(), "observed_tp": data_dict["time_steps"].clone(),
                  "data_to_predict": data_dict["data"].clone(), "tp_to_predict": data_dict["time_steps"].clone(),
                  "observed_mask": None, "mask_predicted_data": None, "labels": None}
...
def subsample_observed_data(data_dict, n_tp_to_sample=None, n_points_to_cut=None):
    # n_tp_to_sample -- if not None, randomly subsample the time points. The resulting timeline has n_tp_to_sample
    # points n_points_to_cut -- if not None, cut out consecutive points on the timeline.  The resulting timeline has
    # (N - n_points_to_cut) points
...
def compute_loss_all_batches(model,
    ...
    total = {"loss": 0, "likelihood": 0, "mse": 0, "kl_first_p": 0, "std_first_p": 0, "pois_likelihood": 0,
             "ce_loss": 0}
    ...
    for i in range(n_batches):
        ...
        if args.classif:
            n_labels = batch_dict["labels"].size(-1)  # model.n_labels
        ...
        if max_samples_for_eval is not None:
            if n_batches * batch_dict["observed_data"].size(0) >= max_samples_for_eval:
                break
