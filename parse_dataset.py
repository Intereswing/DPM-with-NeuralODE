...
from data_process.generate_timeseries import Periodic_1d
from data_process.mujoco_physics import HopperPhysics
from data_process.physionet import PhysioNet, variable_time_collate_fn, get_data_min_max
from data_process.person_activity import PersonActivity, variable_time_collate_fn_activity
...
def parse_datasets(args, device):
    ...
    if dataset_name == "physionet":
        ...
        if not args.classif:
            ...
            # Only 'training' physionet samples are have labels. Therefore, if we do classification task,
            # we don't need physionet 'test' samples.
    ...
    # 最后得到一个长度为n_total_tp的一维tensor，每一个元素服从Unif[0, max_t_extrap]。
    ...
    # dataloader长度不够，用inf_generator来凑
    data_objects = {  # "dataset_obj": dataset_obj,
        "train_dataloader": utils.inf_generator(train_dataloader),
        "test_dataloader": utils.inf_generator(test_dataloader),
        "input_dim": input_dim,
